  ...
  function makeCurriculum(curriculumInfo){
	  ...
	  claCurriculum.appendChild(littleTitle);
		...
		for( i = 0 ; i < curriculumInfo.length; i++){
			...
			let curNameDiv = createDiv("curNameDiv["+i+"]", null,null,null);
			curList.appendChild(curNameDiv);
			curNameDiv.appendChild(createDiv("curNameTitle["+i+"]", null , null , "커리큘럼명"));
			curNameDiv.appendChild(createDiv("curNameInfo["+i+"]", null , null ,curriculumInfo[i].curName));
			
			let curInfoDiv = createDiv("curInfoDiv["+i+"]", null,null,null);
			curList.appendChild(curInfoDiv);
			curInfoDiv.appendChild(createDiv("curInfoTitle["+i+"]", null , null , "커리큘럼내용"));
			curInfoDiv.appendChild(createDiv("curInfoInfo["+i+"]", null, null, curriculumInfo[i].curInfo));
			
			let curDateDiv = createDiv("curDateDiv["+i+"]", null,null,null);
			curList.appendChild(curDateDiv);
			curDateDiv.appendChild(createDiv("curDateTitle["+i+"]", null , null , "커리큘럼기간"));
			
			curDateDiv.appendChild(createDiv("curDateInfo["+i+"]", null, null, curriculumInfo[i].curStartDate.substr(0, 10) +"~"+ curriculumInfo[i].curEndDate.substr(0, 10)));
			
			let curTimeDiv = createDiv("curTimeDiv["+i+"]", null,null,null);
			curList.appendChild(curTimeDiv);
			curTimeDiv.appendChild(createDiv("curTimeTitle["+i+"]", null , null , "커리큘럼기간"));
			
			curTimeDiv.appendChild(createDiv("curTimeInfo["+i+"]", null, null, curriculumInfo[i].curTime + "시간"));
